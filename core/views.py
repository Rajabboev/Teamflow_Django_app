...
class DashboardView(LoginRequiredMixin, TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context["overdue_count"] = (
            tasks.filter(deadline__lt=now).exclude(status="done").count()
        )
...
class TeamListView(LoginRequiredMixin, ListView):
    ...
    def get_queryset(self):
        user = self.request.user
        return Team.objects.filter(Q(created_by=user) | Q(members=user)).distinct()
...
class TaskDetailView(LoginRequiredMixin, DetailView):
    ...
    def get_queryset(self):
        return (
            ...
            .distinct()
            .prefetch_related("feedbacks", "feedbacks__author", "tags")
        )

    def get_context_data(self, **kwargs):
        context = super().get_context_data(**kwargs)
        context["can_give_feedback"] = (
...
class FeedbackCreateView(
    TeamLeadRequiredMixin, LoginRequiredMixin, SuccessMessageMixin, CreateView
):
    model = Feedback
